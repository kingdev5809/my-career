...
const Main = ({ children, handleSubmit, submitForm, values, isSubmitting, setFieldValue, setFieldError, setFieldTouched, errors }) => {
	return (
...
const EnhacedForm = withFormik({
	...
	validationSchema: ({ fields, t }) => {
		...
		fields.forEach(field => {
			...
			if (field.required) {
				validationField = validationField.required(t(`required-${field.name}`));
			}
